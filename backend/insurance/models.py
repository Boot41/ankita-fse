...
class CustomUser(AbstractUser):
    """
    Extended user model that stores additional user details
    beyond what's provided by Django's built-in User model.
    """
    ...
    medical_conditions = ArrayField(
        models.CharField(max_length=50, choices=MEDICAL_CONDITION_CHOICES),
        blank=True,
        null=True,
        help_text="List of user's medical conditions"
    )
...
class InsurancePlan(models.Model):
    """
    Model for storing information about different insurance plans
    ...
    network_hospitals = ArrayField(
        models.CharField(max_length=255),
        blank=True,
        null=True,
        help_text="List of network hospitals"
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text="Whether this plan is currently active and can be recommended"
    )
    
    popularity_score = models.FloatField(
        default=0.0,
        validators=[MinValueValidator(0), MaxValueValidator(5)],
        help_text="Plan popularity score based on user feedback and selections"
    )
    ...
    def __str__(self) -> str:
        """Return a string representation of the InsurancePlan."""
...
class PlanComparison(models.Model):
    ...
    def __str__(self) -> str:
        return f"Comparison {self.comparison_name or self.id} by {self.user.username}"
...
class UserDashboardPreference(models.Model):
    ...
    widgets_order = ArrayField(
        models.CharField(max_length=50),
        default=list,
        help_text="Order of dashboard widgets"
    )
